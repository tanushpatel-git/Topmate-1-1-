...
const Navbar = ({ theam = "white" }) => {
  ...
  const keepMenuOpen = () => clearTimeout(closeTimer.current);
        ...
        <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-2 relative pl-3 pr-3 pt-2 pb-2">
              ...
              >
                {displayItem === "products" && (
                  <motion.div layoutId="nav-bg" className={`absolute inset-0 rounded-full ${isDark ? "bg-white/20" : "bg-black/10"}`} transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                )}
                <button className={`relative z-10 ${textMain} px-4 py-1 rounded-full text-sm`}>Products ▾</button>
              </div>
              ...
              >
                ...
                <button className={`relative z-10 ${textMain} px-4 py-1 rounded-full text-sm`}>Use Cases ▾</button>
              </div>
              ...
              >
                ...
                <div className={`relative z-10 ${textMain} px-4 py-1 rounded-full text-sm`}>Search</div>
              </Link>

              {/* PRICING */}
              <Link to="/pricing" className="relative block"
              ...
              >
                ...
                <div className={`relative z-10 ${textMain} px-4 py-1 rounded-full text-sm`}>Pricing</div>
              </Link>
                      ...
                      <div className="grid grid-cols-3 gap-8">
                          ...
                          <div className="space-y-1">
                            {menuData.map((item, index) => (
                              ...
                              >
                                ...
                                <div>
                                  <h4 className={`text-sm ${activeChild === item.name ? "text-blue-500" : textMain}`}>{item.name}</h4>
                        ...
                        {activeMenu === "features" && (
                          <div className={`col-span-2 ${cardBg} rounded-xl p-4`}>
                            ...
                            <h4 className={`${textMain} text-sm mb-2`}>TOPMATE ECOSYSTEM</h4>
                            <p className="text-gray-400 text-xs mb-4">Everything you need to turn your knowledge into a global business.</p>
                            <button className="text-blue-400 text-sm hover:underline">Explore All Tools →</button>
                          </div>
                        )}
                        ...
                        {activeMenu === "usecases" && (
                          <div className={`col-span-2 ${cardBg} rounded-xl p-4`}>
                            ...
                            <h4 className={`${textMain} text-sm mb-2`}>TOPMATE Use Case</h4>
                            <p className="text-gray-400 text-xs mb-4">The thing which turn u into an expert by knowing use cases</p>
                            <button className="text-blue-400 text-sm hover:underline">Explore All Use Case →</button>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="flex items-center gap-4">
            {userAccess ? (
              ...
              <div className="flex items-center gap-4">
                <Link to="/signin" className={`${buttonSecondary} px-5 py-2 rounded-lg hidden md:block transition`}>Sign In</Link>
                <Link to="/signup" className={`${buttonPrimary} px-5 py-2 rounded-lg hidden md:block`}>Start Selling</Link>
              </div>
            )}

            {/* HAMBURGER */}
            <button onClick={() => setMobileOpen(!mobileOpen)} className={`md:hidden ${textMain}`}>
          ...
          >
            <div className="px-6 py-6 space-y-4">
              ...
              <button onClick={() => setMobileSection(mobileSection === "products" ? null : "products")}
                ...
                <ChevronDown size={18} className={`transition ${mobileSection === "products" ? "rotate-180" : ""}`} />
              </button>
              {mobileSection === "products" && (
                <div className="pl-4 space-y-2">
                  {features.map((item, i) => (
                    <Link key={i} to={item.linkPos} className={`flex items-center justify-between text-gray-400 ${isDark ? "hover:text-white" : "hover:text-black"}`}>
              ...
              <button onClick={() => setMobileSection(mobileSection === "usecases" ? null : "usecases")}
                className="flex justify-between items-center w-full text-left">
                <span className={textMain}>Use Cases</span>
                <ChevronDown size={18} className={`transition ${mobileSection === "usecases" ? "rotate-180" : ""}`} />
              </button>
              ...
              <Link to="/search" className={`flex items-center gap-2 text-gray-400 ${isDark ? "hover:text-white" : "hover:text-black"}`}>
                <Search size={18} />Search
              </Link>

              <Link to="/pricing" className={`text-gray-400 ${isDark ? "hover:text-white" : "hover:text-black"} block`}>
                Pricing
              </Link>

              {/*  Mobile Dashboard is now a <Link>, not a <button> */}
              {userAccess ? (
                <Link to="/creator-dashboard" className={`${buttonPrimary} w-full py-2 rounded-lg mt-4 text-center block`}>
                  Dashboard
                </Link>
              ) : (
                <div className="flex items-center gap-4">
                  <Link to="/signin" className={`${buttonSecondary} w-full py-2 rounded-lg mt-4 text-center transition`}>Sign In</Link>
                  <Link to="/signup" className={`${buttonPrimary} w-full py-2 rounded-lg mt-4 text-center`}>Start Selling</Link>
                </div>
              )}
